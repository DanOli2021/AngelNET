...
class universal:
    ...
    def get_universal_users(self):
            ...
            if maindata["where"] != "null":
               string_where = " AND " + maindata["where"]

            return self.db.Prompt("SELECT * FROM users PARTITION KEY " + partition_key + " WHERE account = '" + account + "'" + string_where, True)          
    ...
    def create_universal_account(self):
            ...
            if result != "{}":
               maindata = json.loads(result)

               if maindata[partition_key][0]["password"] == universalaccount["password"]:
                  return "Ok.The account was confirmed, no changes were made, on the account parameters: " + universalaccount["create_universal_account"] 
            ...
            self.db.Prompt( "CREATE ACCOUNT " + universalaccount["create_universal_account"] + " SUPERUSER " + universalaccount["user"] + " PASSWORD " + universalaccount["password"], True ) 
    ...
    def get_universal_account(self):
            maindata = json.loads(self.data)
            account = maindata["get_universal_account"]
            partition_key = account[0:3]

            self.db.Prompt("USE accounts DATABASE accountsdb" )
            result = self.db.Prompt("SELECT * FROM accounts PARTITION KEY " + partition_key + " WHERE id = '" + maindata["get_universal_account"] + "'", True)

            if result == "{}":
               return "Error: User or password is incorrect" + maindata["get_universal_account"] 
               
            userdata = json.loads( result )

            if userdata[partition_key][0]["password"] != maindata["password"]:
               return "Error: User or password is incorrect: " + maindata["get_universal_account"]
    ...
    def disable_universal_user(self):
            maindata = json.loads(self.data)
            account = maindata["disable_universal_user"].split("@")[1]
            partition_key = account[0:3]

            self.db.Prompt("USE accounts DATABASE accountsdb", True )
            result = self.db.Prompt("SELECT * FROM users PARTITION KEY " + partition_key + " WHERE id = '" + maindata["disable_universal_user"] + "'", True)    

            if result == "{}":
                return "Error: User does not exist: " + maindata["disable_universal_user"]

            result = self.db.Prompt("UPDATE users PARTITION KEY " + partition_key + " SET enabled = '0' WHERE id = '" + maindata["disable_universal_user"] + "'", True)

            return result

    def delete_universal_user(self):
            maindata = json.loads(self.data)
            account = maindata["delete_universal_user"].split("@")[1]
            partition_key = account[0:3]

            self.db.Prompt("USE accounts DATABASE accountsdb", True )
            result = self.db.Prompt("SELECT * FROM users PARTITION KEY " + partition_key + " WHERE id = '" + maindata["delete_universal_user"] + "'", True)    

            if result == "{}":
                return "Error: User does not exist: " + maindata["delete_universal_user"]

            result = self.db.Prompt("DELETE FROM users PARTITION KEY " + partition_key + " WHERE id = '" + maindata["delete_universal_user"] + "'", True)

            return result

    def save_universal_message( self ):
            maindata = json.loads(self.data)
            account = maindata["account"]
            partition_key = account[0:3]
            account_to = maindata["to_account"]
            partition_key_to = account_to[0:3]

            self.db.Prompt("USE accounts DATABASE accountsdb", True )
            self.db.Prompt( "CREATE TABLE chats FIELD LIST account, account_to, data", True )
            
            result = self.db.Prompt("SELECT * FROM accounts PARTITION KEY " + partition_key + " WHERE id = '" + maindata["account"] + "'", True)    

            if result == "{}":
                return "Error: Account does not exist: " + account

            if maindata["to"] != "null":
                account_to = maindata["to"].split("@")[1]
                partition_key_to = account_to[0:3]

                result = self.db.Prompt("SELECT * FROM users PARTITION KEY " + partition_key_to + " WHERE id = '" + maindata["to_account"] + "'", True)    

                if result == "{}":
                    return "Error: Account does not exist: " + account_to

            self.db.Prompt("INSERT INTO chats PARTITION KEY " + partition_key + " VALUES " + maindata, True )
            return "Ok."
