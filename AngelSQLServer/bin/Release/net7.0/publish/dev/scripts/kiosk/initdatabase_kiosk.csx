...
message_data.Add("UserGroup", "KIOSK");
message_data.Add("Name", "KIOSK GROUP");
...
result = db.Prompt($"POST {servers["tokens_url"]} API tokens/admintokens MESSAGE {JsonConvert.SerializeObject(message_data, Formatting.Indented)}");
Console.WriteLine("User Groups --> " + result);

message_data.Clear();
message_data.Add("OperationType", "CreateUser");
message_data.Add("User", "myuser");
message_data.Add("Password", "mysecret");
message_data.Add("UserGroup", "KIOSK");
message_data.Add("Name", "KIOSK USER");
...
if (responce.result.StartsWith("Error:"))
{
    ...
    message_data.Add("User", "myuser");
    message_data.Add("expiry_days", 300000);
    message_data.Add("db_user", "db");
    message_data.Add("db_password", "db");
    result = db.Prompt($"POST {servers["tokens_url"]} API tokens/admintokens MESSAGE {JsonConvert.SerializeObject(message_data, Formatting.Indented)}");
...
result = db.Prompt($"POST {servers["skus_url"]} API kiosk/adminskus MESSAGE {JsonConvert.SerializeObject(operation, Formatting.Indented)}");
Console.WriteLine("Insert standard Clasifications for skus --> " + result);
